...
#doorpage.doorpage(
    ...
    (([Kinokov#footnote[https://github.com/kinoko0518/]], [Main Contributor]), ([Gramme#footnote[https://github.com/gramme-linkcom]], [Contributor])),
    [
...
= 設計と実装
本パーサーの各処理段階における入出力の型シグネチャを定式化すると、以下の通りに表すことができる。Tはテキスト、Dは依存関係にあるすべての画像の名前と画像データのHashMap、Mはメタデータ、Iは中間表現、Oは失敗の可能性を表すモナドである。
...
- Br: 改行に対応。
- Text: テキストのスライスに対応。
Text以外にマッチするパーサーをまとめてSpecialとし、Specialにマッチしうる記号が出現するまで読み飛ばし、Specialにマッチしたらそこに到達するまでの文字列スライスをTextトークンとして追加してからSpecialを追加、最後に長さ0の文字列スライスを無視する方式でTokenize層を実装した。

特筆すべき点として、Textトークンはオリジナルのテキストのスライスのみを持つゼロコピー設計であることにより、メモリアロケーションを削減している。
